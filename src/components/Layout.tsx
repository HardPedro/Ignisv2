    ...
    <>
      ...
      <div className="relative z-10 p-6 flex flex-col h-full justify-end text-white">
        ...
        <div className="mb-6">
          ...
          <p className="text-sm text-yellow-500 font-medium">{user?.role || 'Administrador'}</p>
          <p className="text-xs text-gray-400 mt-1">{user?.tenant_id || 'Oficina Pro'}</p>
        </div>
        ...
        <div className="space-y-3">
          <button onClick={onLogout} className="w-full flex items-center justify-center py-2.5 px-4 rounded-xl border border-gray-600 hover:bg-gray-800 transition-colors text-sm font-medium">
            <Users className="h-4 w-4 mr-2" />
            Trocar Usuário
          </button>
          <button onClick={onLogout} className="w-full flex items-center justify-center py-2.5 px-4 rounded-xl bg-red-500/10 text-red-400 hover:bg-red-500/20 transition-colors text-sm font-medium border border-red-500/20">
    ...
    <div className="flex h-screen bg-gray-50 font-sans overflow-hidden">
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 h-16 bg-gray-900 z-50 flex items-center justify-between px-4 shadow-md">
        <div className="flex items-center">
          {!isHome && (
            <Link to="/" className="mr-3 text-gray-400 hover:text-white">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden bg-gray-50 pt-16 lg:pt-0 relative">
        {/* Top bar for desktop */}
        {!isHome && (
          <div className="hidden lg:flex absolute top-0 left-0 p-6 z-10">
            <Link 
              to="/" 
